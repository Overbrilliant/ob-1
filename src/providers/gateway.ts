...
// OpenAI-compatible routes: managed OB-1 server, the embedded free-models router, or Custom API. This module also owns the
// upstream-error retry policy.
import type { CallOpts, ModelResponse } from "./types.ts";
import { callOpenAI } from "./openai.ts";
import { callFree } from "./free/index.ts";
...
function maxAttempts(): number {
  return Math.max(1, Number(process.env.OB1_MAX_RETRIES) || 10);
}
...
export async function callModel(
  ...
  const dispatch =
    _dispatch ??
    ((o: CallOpts) => {
      // The in-process free-models router does its OWN candidate failover; this outer retry loop stays the
      // backoff layer for full exhaustion (callFree throws an "API 429: …" error → isRetryable → back off).
      if (o.provider === "free") return callFree(o);
