...
// solo: ok,bad -> 0.5 @ 100 tok | fusion: ok,ok -> 1.0 @ 300 tok | codeact: bad,ok -> 0.5 @ 600 tok
...
const outcomes = await runEval({
  ...
  runners: { solo: scripted(["ok", "bad"], 50, 50), fusion: scripted(["ok", "ok"], 150, 150), codeact: scripted(["bad", "ok"], 300, 300) },
});
...
const solo = get("solo"), fusion = get("fusion"), codeact = get("codeact");
...
// codeact: k=600/100=6, solo@k=1-0.5^6=0.984375, pass 0.5 < that -> NOT justified
check("codeact: k=6, solo@k=0.984375, NOT justified", near(codeact.k, 6) && near(codeact.soloAtK, 0.984375) && codeact.justified === false);
